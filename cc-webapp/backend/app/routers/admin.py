...
async def update_system_settings(
    ...
    for k, v in settings.items():
        ...
        if setting:
            ...
            db.add(models.Setting(key=k, value=str(v), updated_at=datetime.utcnow()))
    db.commit()
    updated = db.query(models.Setting).all()
...
async def delete_user(
    ...
    db.delete(user)
    db.commit()
    logger.info("Admin %s deleted user %s", admin.id, target_user_id)
...
async def update_user_info(
    ...
    db.commit()
    logger.info("Admin %s updated user %s info", admin.id, target_user_id)
...
async def get_all_users(
    ...
    logger.info("Admin %s retrieved %d users (page %d)", admin.id, len(user_summaries), page)
...
async def get_user_detail(
    ...
    logger.info("Admin %s viewed user detail for user %s", admin.id, target_user_id)
    return UserDetail(
        id=user.id,
        site_id=user.phone_number,  # phone_number를 사이트ID로 사용
        nickname=user.nickname,
...
async def update_user_rank(
    ...
    user.rank = new_rank
    db.commit()
    logger.info("Admin %s changed user %s rank from %s to %s", admin.id, target_user_id, old_rank, new_rank)
...
async def give_cyber_tokens(
    ...
    db.add(action)
    db.commit()
    logger.info("Admin %s gave %d cyber tokens to user %s", admin.id, request.amount, request.user_id)
    return {
...
async def give_gift_card(
    ...
    db.add(action)
    db.commit()
    logger.info("Admin %s gave %s gift card (%d원) to user %s", admin.id, request.card_type, request.amount, request.user_id)
    return {
...
async def give_shop_item(
    ...
    db.add(action)
    db.commit()
    logger.info("Admin %s gave shop item '%s' (value: %d) to user %s", admin.id, request.item_name, request.item_value, request.user_id)
    return {
